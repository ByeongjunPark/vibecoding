...
<body>
  ...
  <script>
    function submitGuestbook() {
      ...
      { type: 'concept', icon: '🤖', title: '✨ [바이브코딩 확장 실습] AI에게 주문해서 내 맘대로 기능 업그레이드하기!', body: `
        방금 만든 기본 방명록에 <b>"학생 이름으로 내가 작성한 방명록 내역을 팝업창으로 찾아보는 기능"</b>을 추가하고 싶으신가요?<br>
        코드를 직접 한 줄 한 줄 고칠 필요가 전혀 없습니다! 제미나이나 AI 챗봇에게 아래 <b>바이브코딩 프롬프트</b>를 그대로 복사해서 주문해보세요. AI가 완벽하게 기능이 추가된 업그레이드 코드를 짜줍니다!
      ` },
      { type: 'code', filename: '💡 바이브코딩 기능 추가 프롬프트 (복사해서 AI 챗봇에 붙여넣기)', language: 'markdown', code: `다음은 내가 구글 시트 앱스스크립트로 만든 기본 '학생 방명록' 웹앱 코드야.
기존 코드에 [학생 이름으로 작성한 방명록 내역 조회 팝업(모달창)] 기능을 추가해줘!

[요구사항]
1. 뒷단 코드(Code.gs):
   - 학생 이름을 매개변수로 받아 해당 학생이 쓴 모든 방명록 목록(작성일시, 내용)을 찾아서 배열로 반환하는 getGuestbookByName(name) 함수를 추가해줘.

2. 화면 코드(Index.html):
   - 방명록 입력카드 상단이나 하단에 [🔍 내 방명록 내역 조회하기] 버튼을 만들어줘.
   - 버튼을 누르면 이름을 입력하는 창이 뜨고, 백엔드 함수를 호출하여 해당 학생이 남긴 과거 방명록들을 예쁜 팝업창(모달창)으로 리스트 형태로 보여줘.
   - 기존의 방명록 작성 및 저장 기능은 그대로 잘 작동하게 유지해줘.

[기존 Code.gs 및 Index.html 코드]
(※ 위에서 작성한 Code.gs와 Index.html 코드를 여기에 함께 붙여넣어주세요!)` },
      { type: 'tip', variant: 'tip', content: '💡 <b>바이브코딩 핵심 노하우:</b> AI에게 새로운 기능을 추가해 달라고 할 때는 <b>"기존의 잘 작동하는 코드"</b>를 함께 복사해서 보여준 뒤, <b>"이 코드에 ~한 기능을 추가해줘"</b>라고 주문하면 기존 로직이 망가지지 않고 깔끔하게 연동 확장됩니다!' },
      { type: 'divider' },
